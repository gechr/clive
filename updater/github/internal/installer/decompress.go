...
func unzip(src io.Reader, cmd, os, arch string) (io.Reader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w zip file: %w", ErrCannotDecompressFile, err)
	}
...
func untar(src io.Reader, cmd, os, arch string) (io.Reader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w tar.gz file: %w", ErrCannotDecompressFile, err)
	}
...
func gunzip(src io.Reader, cmd, os, arch string) (io.Reader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w gzip file: %w", ErrCannotDecompressFile, err)
	}

	name := r.Name
	if !matchExecutableName(cmd, os, arch, name) {
		return nil, fmt.Errorf(
			"%w: expected %q but found %q",
			ErrExecutableNotFoundInArchive,
			cmd,
			name,
		)
...
func untarxz(src io.Reader, cmd, os, arch string) (io.Reader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w tar.xz file: %w", ErrCannotDecompressFile, err)
	}

	return unarchiveTar(xzip, cmd, os, arch)
}

func unxz(src io.Reader, cmd, _, _ string) (io.Reader, error) {
	log.Print("Decompressing xzip file")

	xzip, err := xz.NewReader(src)
	if err != nil {
		return nil, fmt.Errorf("%w xzip file: %w", ErrCannotDecompressFile, err)
	}
...
func unbz2(src io.Reader, cmd, _, _ string) (io.Reader, error) {
