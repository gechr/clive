...
// You can pass an empty GitHubConfig{} to use the default configuration.
...
func NewGitHubSource(config GitHubConfig) (*GitHubSource, error) {
	...
	if config.EnterpriseBaseURL != "" {
		...
		opts = append(opts, github.WithEnterpriseURLs(config.EnterpriseBaseURL, uploadURL))
	}
...
func (s *GitHubSource) DownloadReleaseAsset(
	...
	// use a new http client so the GitHub library can download the redirected file (if any)
	client := http.DefaultClient
	rc, _, err := s.api.Repositories.DownloadReleaseAsset(ctx, owner, repo, assetID, client)
	if err != nil {
		return nil, fmt.Errorf(
			"failed to call GitHub Releases API for getting the asset ID %d on repository '%s/%s': %w",
			assetID,
			owner,
			repo,
			err,
